...
<style>
  :root {
    --theme-color: #ec4899; /* Rosa da Assistência Social */
  }
</style>
...
<aside class="pbi-sidebar">
  ...
  <!-- Social -->
  <div class="pbi-nav-btn theme-soc active" title="Social" onclick="window.location.href='../assistencia/'" style="cursor:pointer;">
...
<div class="dashboard-container">
  ...
  <div class="dash-header">
    <div class="dash-title-group">
      <h1>Indicadores de Assistência Social e Cidadania</h1>
  ...
  <div class="kpi-grid">
    <div class="kpi-card">
      ...
      <span class="kpi-title">FAMÍLIAS ATENDIDAS</span>
      <h3 class="kpi-value">145K</h3>
      <span class="kpi-trend">+3,2% vs. mês anterior</span>
    </div>
    <div class="kpi-card">
      <div class="kpi-icon">🍽️</div>
      <span class="kpi-title">SEGURANÇA ALIMENTAR</span>
      <h3 class="kpi-value">85%</h3>
      <span class="kpi-trend">+2,5% vs. mês anterior</span>
    </div>
    <div class="kpi-card">
      <div class="kpi-icon">🏠</div>
      <span class="kpi-title">AUXÍLIO MORADIA</span>
      <h3 class="kpi-value">12.500</h3>
      <span class="kpi-trend">-1,1% vs. mês anterior</span>
    </div>
    <div class="kpi-card">
      <div class="kpi-icon">🫂</div>
      <span class="kpi-title">INCLUSÃO SOCIAL</span>
      <h3 class="kpi-value">4.200</h3>
      <span class="kpi-trend">+5,4% vs. mês anterior</span>
    </div>
  </div>
  ...
  <div class="charts-grid">
    <div class="chart-card">
      <div class="chart-header">
        <div>
          <div class="chart-title">Atendimentos Mensais</div>
          <div class="chart-subtitle">Evolução mensal — Jan a Ago 2024</div>
        </div>
        <span class="chart-badge">Acumulado 2024</span>
      </div>
      <div class="chart-body">
        <canvas id="lineChartAssistencia"></canvas>
      </div>
    </div>
    <div class="chart-card">
      <div class="chart-header">
        <div>
          <div class="chart-title">Programas Sociais</div>
          <div class="chart-subtitle">Distribuição de Benefícios</div>
        </div>
      </div>
      <div class="chart-body" style="display: flex; align-items: center; justify-content: center;">
        <canvas id="donutChartAssistencia"></canvas>
      </div>
    </div>
  </div>
  ...
  <div class="charts-grid">
    <div class="chart-card">
      <div class="chart-header">
        <div>
          <div class="chart-title">Índice de Vulnerabilidade Coberta (%)</div>
          <div class="chart-subtitle">Jan - Ago 2024</div>
        </div>
      </div>
      <div class="chart-body">
        <canvas id="barChartAssistencia"></canvas>
      </div>
    </div>
    <div class="chart-card">
      <div class="chart-header">
      ...
      <div class="alert-list">
        <div class="alert-item">
          <div class="alert-label"><span class="alert-icon ok">✓</span> Cobertura CRAS</div>
          <div class="alert-meta">Meta: > 80% <span class="alert-value">84%</span></div>
        </div>
        <div class="alert-item">
          <div class="alert-label"><span class="alert-icon ok">✓</span> Benefícios Entregues no Prazo</div>
          <div class="alert-meta">Meta: > 90% <span class="alert-value">92%</span></div>
        </div>
        <div class="alert-item">
          <div class="alert-label"><span class="alert-icon warn">⚠</span> Fila de Espera Cadastro Único</div>
          <div class="alert-meta">Meta: < 5.000 <span class="alert-value" style="color:#f59e0b">6.800</span></div>
        </div>
      </div>
    </div>
  </div>
...
<script>
  document.addEventListener("DOMContentLoaded", function() {
    ...
    new Chart(document.getElementById('lineChartAssistencia'), {
      type: 'line',
      data: {
        labels: ['Jan', 'Fev', 'Mar', 'Abr', 'Mai', 'Jun', 'Jul', 'Ago'],
        datasets: [{
          label: 'Atendimentos', data: [120, 125, 130, 132, 135, 140, 142, 145], borderColor: '#ec4899', tension: 0.4, fill: true, backgroundColor: 'rgba(236, 72, 153, 0.1)'
        }]
      },
    ...
    new Chart(document.getElementById('donutChartAssistencia'), {
      type: 'doughnut',
      data: {
        labels: ['Bolsa Família/Estadual', 'Auxílio Gás', 'BPC', 'Outros'],
        datasets: [{ data: [55, 20, 15, 10], backgroundColor: ['#ec4899', '#8b5cf6', '#38bdf8', '#f59e0b'], borderWidth: 0 }]
      },
      options: { responsive: true, maintainAspectRatio: false, cutout: '70%', plugins: { legend: { position: 'right' } } }
    });

    new Chart(document.getElementById('barChartAssistencia'), {
      type: 'bar',
      data: {
        labels: ['Jan', 'Fev', 'Mar', 'Abr', 'Mai', 'Jun', 'Jul', 'Ago'],
        datasets: [{ data: [75, 76, 78, 80, 81, 83, 84, 85], backgroundColor: '#ec4899', borderRadius: 4 }]
      },
