...
<style>
  :root {
    --theme-color: #f59e0b; /* Amarelo/Laranja da Segurança */
  }
</style>
...
<aside class="pbi-sidebar">
  ...
  <!-- Segurança -->
  <div class="pbi-nav-btn theme-seg active" title="Segurança" onclick="window.location.href='../seguranca/'" style="cursor:pointer;">
...
<div class="dashboard-container">
  ...
  <div class="kpi-grid">
    <div class="kpi-card">
      ...
      <span class="kpi-title">OCORRÊNCIAS/MÊS</span>
      <h3 class="kpi-value">1.420</h3>
      <span class="kpi-trend negative">-23% vs. mês anterior</span>
    </div>
    <div class="kpi-card">
      <div class="kpi-icon">🛡️</div>
      <span class="kpi-title">TAXA DE RESOLUÇÃO</span>
      <h3 class="kpi-value">80%</h3>
      <span class="kpi-trend">+17,6% vs. mês anterior</span>
    </div>
    <div class="kpi-card">
      <div class="kpi-icon">👮</div>
      <span class="kpi-title">EFETIVO POLICIAL</span>
      <h3 class="kpi-value">8.200</h3>
      <span class="kpi-trend">+1,5% vs. mês anterior</span>
    </div>
    <div class="kpi-card">
      <div class="kpi-icon">🚓</div>
      <span class="kpi-title">PATRULHAMENTO</span>
      <h3 class="kpi-value">84%</h3>
      <span class="kpi-trend">+16,6% vs. mês anterior</span>
    </div>
  </div>
  ...
  <div class="charts-grid">
    <div class="chart-card">
      <div class="chart-header">
        <div>
          <div class="chart-title">Ocorrências & Resolução</div>
          <div class="chart-subtitle">Evolução mensal — Jan a Ago 2024</div>
        </div>
        <span class="chart-badge">Acumulado 2024</span>
      </div>
      <div class="chart-body">
        <canvas id="lineChartSeguranca"></canvas>
      </div>
    </div>
    <div class="chart-card">
      <div class="chart-header">
        <div>
          <div class="chart-title">Status das Ocorrências</div>
      ...
      <div class="chart-body" style="display: flex; align-items: center; justify-content: center;">
        <canvas id="donutChartSeguranca"></canvas>
      </div>
    </div>
  </div>
  ...
  <div class="charts-grid">
    <div class="chart-card">
      <div class="chart-header">
        <div>
          <div class="chart-title">Taxa de Patrulhamento (%)</div>
          <div class="chart-subtitle">Jan - Ago 2024</div>
        </div>
      </div>
      <div class="chart-body">
        <canvas id="barChartSeguranca"></canvas>
      </div>
    </div>
    <div class="chart-card">
      <div class="chart-header">
      ...
      <div class="alert-list">
        <div class="alert-item">
          <div class="alert-label"><span class="alert-icon warn">⚠</span> Homicídios por 100k hab.</div>
          <div class="alert-meta">Meta: < 15 <span class="alert-value" style="color:#f59e0b">18,4</span></div>
        </div>
        <div class="alert-item">
          <div class="alert-label"><span class="alert-icon ok">✓</span> Efetivo Policial Ativo</div>
          <div class="alert-meta">Meta: > 8.000 <span class="alert-value">8.200</span></div>
        </div>
        <div class="alert-item">
          <div class="alert-label"><span class="alert-icon ok">✓</span> Viaturas Operacionais</div>
          <div class="alert-meta">Meta: > 85% <span class="alert-value">87%</span></div>
        </div>
      </div>
    </div>
  </div>
...
<script>
  document.addEventListener("DOMContentLoaded", function() {
    ...
    new Chart(document.getElementById('lineChartSeguranca'), {
      type: 'line',
      data: {
        labels: ['Jan', 'Fev', 'Mar', 'Abr', 'Mai', 'Jun', 'Jul', 'Ago'],
        datasets: [{
          label: 'Ocorrências', data: [1800, 1750, 1700, 1680, 1600, 1550, 1500, 1420], borderColor: '#f59e0b', tension: 0.4
        }, {
          label: 'Resolução', data: [65, 68, 70, 72, 75, 78, 79, 80], borderColor: '#10b981', tension: 0.4
        }]
      },
    ...
    new Chart(document.getElementById('donutChartSeguranca'), {
      type: 'doughnut',
      data: {
        labels: ['Resolvidas', 'Em Andamento', 'Arquivadas'],
        datasets: [{ data: [80, 14, 6], backgroundColor: ['#10b981', '#f59e0b', '#ef4444'], borderWidth: 0 }]
      },
      options: { responsive: true, maintainAspectRatio: false, cutout: '70%', plugins: { legend: { position: 'right' } } }
    });

    new Chart(document.getElementById('barChartSeguranca'), {
      type: 'bar',
      data: {
        labels: ['Jan', 'Fev', 'Mar', 'Abr', 'Mai', 'Jun', 'Jul', 'Ago'],
        datasets: [{ data: [70, 72, 75, 76, 78, 80, 82, 84], backgroundColor: '#10b981', borderRadius: 4 }]
      },
